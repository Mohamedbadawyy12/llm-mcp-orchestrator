...
class MCPHttpClient:
    ...
    This client creates a new httpx.AsyncClient for each operation
    to avoid event loop conflicts when used inside asyncio.run().
    """
        ...
        try:
            async with httpx.AsyncClient(base_url=self.base_url, timeout=self.timeout) as client:
                async with aconnect_sse(
                    client, "POST", f"{self.base_url}/mcp/run", json=request_body
                ) as event_source:
